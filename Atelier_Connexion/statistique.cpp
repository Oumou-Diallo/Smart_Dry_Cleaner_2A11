...
void statistique::stat(QCustomPlot *customPlot)
{
    ...
    fossil->setName("statistique des quantités des équipements");
   ...
   /* query.prepare("SELECT COUNT(DISTINCT cin) FROM gs_personnel where age between 15 and 25");
    ...
    query.prepare("SELECT COUNT(qte_eq) FROM EQUIPEMENT where type_eq like '%'savons lessive'%'");
    query.exec();
    int un;
    while(query.next())
    {
    ...
    query.prepare("SELECT COUNT(qte_eq) FROM EQUIPEMENT where type_eq like '%'eau de javel'%'");
    query.exec();
    int deux;
    while(query.next())
    {
    ...
    query.prepare("SELECT COUNT(qte_eq) FROM EQUIPEMENT where type_eq like '%'machine a lave'%' ");
    query.exec();
    int trois;
    while(query.next())
    {
    ...
    query.prepare("SELECT COUNT(qte_eq) FROM EQUIPEMENT where type_eq like '%'machine a repasser'%' ");
    query.exec();
    int quatre;
    while(query.next())
    {
...
/*void Produit::statistique(QVector<double>* ticks,QVector<QString> *labels)
{
    ...
    ticks << 1 << 2 << 3 << 4;
    labels <<"savons lessive"<< "eau de javel" << "machine a lave"<< "machine a repasser";
    ...
    customPlot->yAxis->setLabel("quantite ");
 ...
 //   regen->setData(ticks, regenData);*/
   ...
   QVector<double> PlaceData;
              QSqlQuery q1("select qte_eq from EQUIPEMENT");
              while (q1.next()) {
